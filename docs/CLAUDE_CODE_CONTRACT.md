...
| Keep the connection alive through Ollama's prompt-eval silence | Ping heartbeat now fires *before* `message_start` too (`anthropic_translator.py:529`), so a byte flows every ~15s during eval — without it, a full-tool opus turn (eval >120s on a 12B) emits nothing and a Cloudflare front-end 524s at its 120s read timeout | High | **Fixed** |
| Retry Ollama's intermittent grammar-cache 400 | `_is_transient_grammar_400` + 3-attempt retry in both native paths (`services.py:_ollama_native_chat`, `:_ollama_native_stream`). The "can't find closing '}'" 400 fires stochastically even on valid sanitized schemas (llama.cpp grammar-cache bug); the same payload succeeds ~100% on retry. Genuine schema/auth 400s still surface at once | High | **Fixed** |
